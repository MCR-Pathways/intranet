...
**Place an `eslint-disable-next-line` for an attribute-level rule directly above the flagged attribute, between attributes — not before the element.** `no-autofocus`, `no-noninteractive-tabindex` and `media-has-caption` report on the offending *attribute*'s line (`autoFocus`, `tabIndex`, the `<video>`), so a `{/* */}` comment before the `<element` opening targets the wrong line — the violation survives AND you get an "Unused eslint-disable directive" warning. A `//` comment between attributes is valid JSX: put `// eslint-disable-next-line jsx-a11y/no-autofocus -- reason` on the line immediately above the attribute. For an element at a `return (` or ternary root (a JS expression position), the same `//` line works between the `(` and the `<element`. When that's awkward, wrap with a `{/* eslint-disable rule */} ... {/* eslint-enable rule */}` block. Element-level rules (`no-static-element-interactions`, `click-events-have-key-events`) DO report on the opening-element line, so a comment immediately before `<div` is correct for those. Always re-run `eslint` after adding a directive and confirm there's no "unused directive" warning — that's the signal it landed on the wrong line.
