...
The full `jsx-a11y` recommended set is enforced at `error` (`buildA11yRules()` in `eslint.config.mjs`, ADR-012). New violations fail CI; clear them or add a justified disable. Patterns the warn-to-error sweep settled:
