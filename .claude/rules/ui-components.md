...
**Use `createElement()` for dynamic Lucide icons to satisfy React Compiler.** `createElement(resolveIcon(name), props)` avoids the `react-hooks/static-components` lint violation. This applies to *any* dynamically-resolved component variable, not only `resolveIcon(...)`: `const Icon = config.Icon; <Icon className="…" />` is the same footgun, so write `createElement(Icon, { className: "…" })`. It holds in Plate static components too — the resource-grid chip missed it and it was caught in review (PR #375). ESLint does not flag every instance, but the codebase applies the pattern uniformly (article-breadcrumb, plate-elements, category-grid, notification-bell), so match it rather than relying on the linter.

**`react-hooks/purity` flags `Date.now()` in render, but not `new Date().getTime()`.** The React Compiler's purity rule rejects `Date.now()` as impure. **In a server component** (renders once per request, with no client-side memoisation of the render) reading the clock inline is genuinely fine, and `new Date().getTime()` is the same value without tripping the rule — it targets the `Date.now` identifier; the `Date` constructor passes. Use it only for that server-side read, e.g. `getDueStatus(new Date(dueDate), new Date().getTime())` in the course-detail page. **Do NOT reach for `new Date().getTime()` to silence the rule in a client component.** There the impurity is real: the Compiler memoises, so reading the clock in render breaks idempotency and can render stale UI — the lint is correct, and the fix is to pass a server-provided `now` as a prop (see the date-calc rule below), not to hide the read behind an un-flagged form.
...
**Tailwind v4's `dark:` variant defaults to `prefers-color-scheme`, not the `.dark` class.** This app is light-only: `globals.css` has a `.dark` token block but nothing ever sets the `.dark` class (no toggle, no next-themes). Without an explicit `@custom-variant dark (&:where(.dark, .dark *))`, every raw `dark:` colour utility fires from the OS colour scheme, so on a dark-OS browser dark chip/banner colours render over the permanently-light token page (the semantic tokens stay light because they are gated on the never-set class). `globals.css` now registers the class-based variant and sets `color-scheme: light` on `:root` (`dark` on the dormant `.dark` block) so native controls follow suit. Treat any `dark:` utility as class-gated (dormant until a real theme toggle sets `.dark`), not automatic. Verify with `eslint`-independent DOM inspection: `getComputedStyle` on the element, and a probe `<div class="bg-red-50 dark:bg-red-950/30">` should read the light value on a dark-OS machine. See ADR-018 and PR #374.
...
## Accessibility (jsx-a11y) lint patterns

The full `jsx-a11y` recommended set is enforced at `error` (`buildA11yRules()` in `eslint.config.mjs`). New violations fail CI; clear them or add a justified disable. Patterns the warn-to-error sweep settled:

**Disabling an attribute-level a11y rule in JSX: prefer a `{/* eslint-disable */} ... {/* eslint-enable */}` block.** `no-autofocus`, `no-noninteractive-tabindex` and `media-has-caption` report on the offending *attribute*'s line (`autoFocus`, `tabIndex`, the `<video>`), not the element's opening line. So a single `{/* eslint-disable-next-line */}` before the `<element` opening misses — it targets the element line, the violation survives, and you get an "Unused eslint-disable directive" warning. The robust fix is a JSX-comment **block** wrapping the element: `{/* eslint-disable jsx-a11y/no-autofocus */}` above `<element` and `{/* eslint-enable jsx-a11y/no-autofocus */}` after the closing tag — it covers the attribute wherever it sits and survives any reformatting. A terser alternative is a `//` line comment *between attributes*, directly above the flagged one (`// eslint-disable-next-line jsx-a11y/no-autofocus -- reason`); it's precise and used across this repo (no Prettier / auto-formatter today), but a line-collapsing formatter could merge it and comment out the following attributes, so reach for the block if that ever changes. Element-level rules (`no-static-element-interactions`, `click-events-have-key-events`) DO report on the opening-element line, so a `{/* eslint-disable-next-line */}` immediately before `<div` is correct for those. Always re-run `eslint` after adding a directive and confirm there's no "unused directive" warning — that's the signal it landed on the wrong line.

**`aria-hidden="true"` removes a decorative element from the interaction rules — no disable needed.** A click-to-close backdrop scrim with an `onClick` trips `no-static-element-interactions` + `click-events-have-key-events`. Adding `aria-hidden="true"` (correct — the scrim is decorative; the real close paths are Escape and the nav) drops it from the a11y tree, so both rules stop firing. Prefer this over a suppression when the element genuinely should not be in the a11y tree.

**Radix `DialogContent`/`PopoverContent` auto-focus the first focusable on open; explicit `autoFocus` is redundant but defensible.** shadcn's `DialogContent` renders `<DialogClose>` (the X) AFTER `{children}` (`ui/dialog.tsx`), so the first form input is the first focusable and Radix lands focus on it. Keeping an explicit `autoFocus` (with its justified `// eslint-disable-next-line jsx-a11y/no-autofocus`, since the rule is at `error`) *declares* the intended initial-focus target rather than inferring it from DOM order — robust to a control being inserted before the input later. Don't remove `autoFocus` on the assumption Radix handles it unless you've confirmed the input is genuinely first-focusable (a control before it makes Radix focus that instead, so removing `autoFocus` would regress). `no-autofocus` also flags `autoFocus` on custom components (e.g. `<TiptapComposer autoFocus>`) by default — suppress at the call site rather than setting `ignoreNonDOM: true`, since the prop forwards to a real focus.

**Honest-gap disables vs false-positive disables — keep them distinct.** Most a11y suppressions dismiss a *false positive*: the element isn't a control (a `stopPropagation` wrapper, a click-to-focus sugar around a keyboard-reachable editor) or the behaviour is intentional and expected (focus-on-open in a user-summoned dialog — the ARIA APG pattern for data-entry dialogs). But occasionally a rule flags a *real* gap with no markup fix — `media-has-caption` on author-uploaded `<video>` with no caption file and no upload flow. The empty `<track>` the rule would accept is a false promise to assistive tech, so don't add it. The honest resolution is a justified disable that *names the gap* plus a tracked backlog item for the real fix (here, a `.vtt` upload rendered as `<track>` when present). A disable that silently accepts a real gap with no logged follow-up is the thing to avoid.
