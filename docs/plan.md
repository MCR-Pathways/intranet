...
- [x] Table standardisation (COMPLETE — 15 of 17 tables migrated, 2 skipped, 7 PRs): Shadcn Table primitives + TanStack Table (`@tanstack/react-table`). Interactive sorting, pagination, sticky headers, row numbering. Foundation PR #92, profile tabs PR #93, HR dashboards PR #94, HR workflows PR #95, HR admin PR #96 (all merged). Sign-in+learning PR #97 (open). 2 tables intentionally skipped: team-schedule-grid (specialised calendar grid), reports-panel (simple embedded report). Research: 10 platforms studied. See `memory/table-standardisation.md`.
