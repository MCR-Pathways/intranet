...
- [x] **P3-F cleanup** — deleted dead code (`ArticleRenderer`, a 388-line Tiptap renderer superseded by Plate per ADR-010; `ResourceSearch`, superseded by the Cmd+K global search, + its test) and dropped 6 redundant `bg-card` overrides on `outline` buttons (the variant fills it natively since ADR-014).
