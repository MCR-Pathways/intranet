...
- [ ] Table standardisation (IN PROGRESS — 6 PRs): Shadcn Table primitives + TanStack Table (`@tanstack/react-table`). Interactive sorting, pagination, sticky headers, row numbering. 17 tables migrated across HR (13), Sign-In (2), Learning (2). PRs: foundation → simple profile → HR dashboards → HR workflows → HR admin → sign-in+learning. Research: 10 platforms studied.
