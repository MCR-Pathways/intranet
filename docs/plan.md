...
- [x] **P3-F cleanup** — deleted dead code (`ArticleRenderer`, a 388-line Tiptap renderer superseded by Plate per ADR-010; `ResourceSearch`, superseded by the Cmd+K global search, + its test) and dropped 10 redundant `bg-card` overrides on `outline` buttons across 6 files (the variant fills it natively since ADR-014; the new `no-bg-card-on-outline` ESLint rule blocks regressions and caught the 4 the grep sweep missed).
- [x] **P3-F StatusBanner — investigated; no component, but a bug-fixing helper extracted.** The audit's "Learning 4× status pills" doesn't hold: the enrolment-status pills (completed / in-progress / enrolled) live in one place (`course-card`), not duplicated; the due-date status renders in two places with *different* shapes — a compact `<Badge>` on `enrolled-course-card` vs a two-line alert block on the course detail page — so no shared UI component fits both. But the due-date *computation* was duplicated, and Gemini's review caught a real boundary bug in it: `Math.ceil` of a sub-day overdue gap returns `-0`, and `-0 < 0` is false, so a course overdue by under 24h was shown as "due soon" (amber) instead of "overdue" (red) on both sites. Extracted a unit-tested `getDueStatus(dueDate, now)` to `src/lib/learning.ts` that classifies overdue on the raw timestamp, fixing it in one place. The `<=14`-day list filters are a separate "upcoming" concept, left as-is. `preview-mode-banner.tsx` is used in 3 places (not dead).
- [~] **P3-F org-chart pass — DROPPED.** Colin is rebuilding the org chart (not working as wished), so the `#94a3b8` connector swap + `bg-muted/30` panel fold into that rebuild, not a standalone fix. See `memory/org-chart-rebuild.md`.
...
- [x] **a11y enforcement** — FOUNDATIONS MERGED (#349 config, #351 truncate). The `jsx-a11y` recommended set + a custom `mcr-a11y/no-truncate-without-title` rule (`eslint-plugin-jsx-a11y` is a direct devDep). **Config correction — ADR-012, PR #352:** the original config mapped `Object.keys(recommended.rules) → "warn"`, which stripped each rule's recommended options (e.g. `control-has-associated-label`'s `ignoreElements`, excluding input/textarea/video) AND force-enabled off-by-default rules — inflating the "143" baseline with false positives AND silently downgrading 24 important error-rules to warn. Corrected to apply the recommended set properly (`buildA11yRules()` in `eslint.config.mjs`): 24 already-clean rules now enforced at **`error`**, options preserved, off-rules off. **Done:** truncate (74 → 0, #351, at error); `control-has-associated-label` (genuine surface was ~1, the icon-picker button — now at error, 0 left); **Slice A quick-wins** — `iframe-has-title` (1 → 0: title on the lesson video embed), `no-noninteractive-tabindex` (1 → 0: the disabled-button tooltip's focusable-proxy span keeps its `tabIndex` under a justified block-disable, the WAI pattern for tooltips on disabled controls), `label-has-associated-control` (3 → 0: the export-poll dialog's nested-`<label>` checkboxes converted to the sibling `Checkbox id` + `Label htmlFor` pattern used across the HR forms) — all three now at error; **Slice B interaction handlers** — `no-static-element-interactions` (8 → 0) + `click-events-have-key-events` (8 → 0), the same 8 elements tripping both. Resolved by capability, not by silencing: 2 genuine controls made keyboard-operable (compliance file-dropzone → `<button>`; onboarding accordion header → `role="button"` + `aria-expanded` + Enter/Space), the mobile-menu backdrop given a real keyboard close (Escape-to-close effect; the scrim itself `aria-hidden`, which also drops it from the rule), and 5 non-interactive wrappers (4 `stopPropagation` containers + the Tiptap click-to-focus sugar) given justified disables since their inner controls are already keyboard-accessible — both rules now at error; **Slice C autofocus** — `no-autofocus` (15 → 0). All 15 were intentional focus-on-open in user-summoned dialogs / popovers / inline forms (for modals, focusing the first field is the ARIA APG pattern), none a page-load focus theft — so each got a one-line justified `eslint-disable` and the rule is promoted to error, which gates any *future* autofocus behind an explicit reason and keeps a clean 0-autofocus lint baseline; **Slice D media captions** — `media-has-caption` (2 → 0): both sites are author-uploaded `<video>` (the lesson player + the resources article renderer) with no caption file and no VTT-upload flow, so the `<track>` the rule wants would be a *false* promise of captions. With nothing to attach, each got a justified `eslint-disable` pointing at the deferred caption feature (below), and the rule is promoted to error so any *new* captionless `<video>` forces a conscious decision. **The whole `jsx-a11y` recommended set is now enforced at `error`** — the colour-rework a11y backlog is closed. `label-has-for` is deprecated/off (its 6 were phantom). Self-tracking: `npm run lint:a11y`.
- [ ] **Video captions** (a11y feature, deferred from the `media-has-caption` slice) — uploaded lesson videos (`video-player.tsx`) and MCR-hosted resources videos (`plate-static-plugins.tsx`) render as native `<video>` with no captions, a real barrier for deaf/HoH users. No caption infrastructure exists today (no `.vtt` storage, no `<track>`, no upload field). Real fix: add an optional `.vtt` upload alongside the video in the lesson/media upload flow, store it, render `<track kind="captions" src=...>` when present, then drop the two `eslint-disable jsx-a11y/media-has-caption` comments. Until then the gap is acknowledged, not hidden.
- [x] **Undefended static-map access** (latent `TypeError`): FIXED via a shared `getCategoryConfig()` helper in `src/lib/learning.ts` (neutral `DEFAULT_CATEGORY_CONFIG` fallback). Swept every undefended `categoryConfig[dbKey]` site — `learning/courses/[id]/page.tsx`, `course-management-table.tsx`, `course-card.tsx`, `enrolled-course-card.tsx`. Left as-is: `external-course-card` (intentionally `&&`-guarded to hide unknown categories) and `actions.ts` ×2 (already `catConfig?.label ?? rawCategory` — switching would degrade the search-indexed label to "Other"). **`noUncheckedIndexedAccess` evaluated and declined as a global flag:** 689 errors, of which 514 (75%) are in test files and 33 in one-off scripts — only 142 in real app code (~21 of those in WP-migration lib). The signal drowns in test/script noise and it would block CI on 689 errors; targeted defensive fixes (the `getCategoryConfig` pattern) handle the genuine crash risks at far lower cost. Re-measure only if test/script noise is ever excluded via a scoped tsconfig.

**Drift-prone but low-value → sweep when convenient, no rule:**
- [x] **`transition-all` → targeted transitions** (DONE): each `transition-all` replaced with a transition that keeps only the property actually animating. 4 width-animators (sidebar collapse `w-16`/`w-64`; the lesson / onboarding / poll progress bars driven by inline `width`) → `transition-[width]`; the Radix progress indicator (inline `transform: translateX`) → `transition-transform`; ~19 paint-only hover sites (colour / bg / border-color / shadow / opacity / transform) → `transition`. Left as `transition-all`: `button` (covers `text-decoration-thickness` + `underline-offset` for the link variant, documented in-file) and `course-header` (animates `px-0`→`px-3` padding on focus) — both legit multi-prop animations where the safe default would drop a needed property. No behaviour change.
...
- Defensive `categoryConfig` access (learning) → DONE via `getCategoryConfig()` helper; `noUncheckedIndexedAccess` evaluated + declined (689 errors, 75% test noise). See Code hygiene & enforcement backlog.
