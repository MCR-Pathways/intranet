...
- [ ] Table standardisation (IN PROGRESS — 6 PRs): Shadcn Table primitives + TanStack Table (`@tanstack/react-table`). Interactive sorting, pagination, sticky headers, row numbering. 17 tables migrated across HR (13), Sign-In (2), Learning (2). Foundation PR #92 (merged). Simple profile migrations PR #93. Remaining: HR dashboards → HR workflows → HR admin → sign-in+learning. Research: 10 platforms studied.
