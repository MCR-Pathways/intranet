...
- [ ] **P3-F StatusBanner** — investigate the Learning status-pill duplication the audit flagged ("4× + preview banner"); a first grep didn't surface an obvious 4× and `preview-mode-banner.tsx` is used in 3 places (not dead), so extract a shared component only if the duplication genuinely earns it. Separate PR.
