...
- [x] **P3-F StatusBanner — investigated, not warranted.** The audit's "Learning 4× status pills" doesn't hold: the enrolment-status pills (completed / in-progress / enrolled) live in one place (`course-card`), not duplicated; the due-date status renders in two places with *different* shapes — a compact `<Badge>` on `enrolled-course-card` vs a two-line alert block on the course detail page — so one shared component would fight both. The only real repetition is the due-date *computation* (`daysUntilDue` + the 7-day `overdue`/`due_soon` thresholds, ~12 lines × 2 sites); a `getDueStatus()` helper could DRY it but is too marginal to earn extraction (the `<=14`-day list filters are a separate "upcoming" concept, intentionally different). `preview-mode-banner.tsx` is used in 3 places (not dead). No component extracted.
