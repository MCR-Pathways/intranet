...
- [~] **a11y enforcement** — FOUNDATION DONE (PR #349). The full `jsx-a11y` recommended set (34 rules, was only 6 via Next core-web-vitals) + a custom `mcr-a11y/no-truncate-without-title` rule + `eslint-plugin-jsx-a11y` as a direct devDep. `jsx-a11y/control-has-associated-label` subsumes the once-planned icon-button-label rule (no custom one needed). All `warn`, so **143 a11y warnings now surface on every lint/CI run** (0 errors). **Remaining sweep:** clear the 143 to zero, batched by rule/module — 74 `no-truncate-without-title` (mostly mechanical `title={text}`), 18 `control-has-associated-label`, 15 `no-autofocus`, 11 `no-static-element-interactions`, 8 `click-events-have-key-events`, +20 across label-association / tabindex / media-captions / iframe-title — then promote each rule to `error`. The warnings are self-tracking: `npm run lint:a11y` shows the live count, so this can't be silently dropped.
