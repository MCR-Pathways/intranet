...
| Date | Author | Summary |
|---|---|---|
| 2026-04-13 | Abdulmuiz Adaranijo | Native editor WS4 media complete (PRs #223, #228). Google Drive upload + authenticated streaming proxy with resource_media whitelist table (migration 00077). Image plugin (dialog upload with progress, clipboard paste with dimensions, alt text editing, delete confirmation). Video embed plugin (YouTube/Vimeo with nocookie privacy mode, sandbox attribute). File attachment plugin (PDF/Word/Excel/PowerPoint/text, 25 MB max, download link). Proxy optimised to use DB metadata instead of redundant Drive API call. Accessibility: aria-labels on media action buttons, iframe titles (WCAG 2.4.1). CLS prevention: width/height attributes on static and editor images, height:auto style. Toolbar visibility fixed: useSelected hook replaces broken group-focus-within on void elements. findPath(element) replaces stale props.path for node mutations. Paste handler: correct MIME-based extension, URL-keyed dimension Map for concurrent paste safety, widened try/catch for blob/fetch failures. formatBytes duplication removed. 23 new tests (19 getEmbedUrl + 4 static edge cases). 3 Gemini review rounds addressed. 1,371 tests, 59 files. |
