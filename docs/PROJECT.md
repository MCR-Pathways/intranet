...
**Global search bar.** An always-visible search bar in the header (replacing the old icon + Cmd+K modal), centred over the feed column. Inline `cmdk` dropdown anchored beneath it; results span three Algolia indices — resources, courses, and news posts — with scope tabs (All / Resources / Courses / News). News covers plain news posts and polls (kudos and weekly round-ups are excluded). The bar mirrors the main content column's sidebar offset, so it tracks the feed centre in both sidebar states; it shows at larger widths only, with a narrower-viewport version deferred to the responsive pass. Cmd/Ctrl+K focuses it, and dismissing (Escape, click-outside, blur) keeps the typed query.
...
| Date | Author | Summary |
|---|---|---|
| 2026-06-18 | Abdulmuiz Adaranijo | Post-a11y cleanup: a hygiene sweep, a Learning bug fix, and the ADR-log reconciliation. The `transition-all` utility (which transitions every animatable property, so an unrelated layout or colour change on an element animates too) was replaced with property-targeted transitions across ~27 sites — `transition-[width]`, `transition-transform`, `transition-colors`, `transition-shadow` per case. The Learning due-date status, duplicated inline on the course-detail page and the enrolled-course card, was extracted into a unit-tested `getDueStatus(dueDate, now)` in `src/lib/learning.ts`, fixing a boundary bug Gemini caught: `Math.ceil` of a sub-day overdue gap returns `-0`, and `-0 < 0` is false, so a course overdue by under 24h showed `due_soon` (amber) instead of `overdue` (red). The helper decides overdue on the raw timestamp, normalises the `-0` to a clean `0` so the page reads "due today" for the sub-day case, and pluralises the day counts ("1 day ago", not "1 days ago"). The P3-F StatusBanner investigation behind it closed as no-component — the two render sites differ in shape (a compact Badge vs a two-line alert block), so one shared component fits neither. The ADR log was then reconciled: the jsx-a11y enforcement decision lived only on the project board (numbered ADR-011/012, which canonically mean TanStack Table and CSP image proxying) and was absent from the canonical `PROJECT.md` log — so canonical gained ADR-015 (Accessibility enforced in ESLint), the board's two mislabelled a11y entries consolidated into one board ADR-015, and the four canonical ADRs the board still lacked (011 TanStack Table, 012 CSP image proxying, 013 component pages, 014 home-feed colour) were mirrored in so the board now renders the full sequence 001-015. Gemini's review surfaced a stale fact in canonical itself — ADR-011 documented the DataTable wrapper as `shadow-sm`, but a 2026-04-17 `md→sm` change was reverted and `data-table.tsx` is back to `shadow-md` — corrected in both docs. The hosted board republish is parked: the `mcr-publish`/`mcr-project` skills are not installed on this machine, so the public copy lags while `docs/project.html` in the repo stays the source of truth. PRs #360–363. |
| 2026-06-18 | Abdulmuiz Adaranijo | Accessibility enforcement completed end-to-end, plus two hardening follow-ups. The full `eslint-plugin-jsx-a11y` recommended set is now a hard `error` gate with zero warnings. The foundation enabled the recommended set (34 rules) as a direct dev dependency alongside a custom `mcr-a11y/no-truncate-without-title` AST rule for CSS truncation, where `truncate`/`line-clamp` hide text with no `title` to read the full string on hover. A config correction fixed the original setup, which had mapped each recommended rule key to `"warn"` — that stripped every rule's recommended options (e.g. `control-has-associated-label`'s `ignoreElements`, which excludes form inputs) and force-enabled rules the recommended set deliberately turns off, inflating the baseline with false positives while silently downgrading 24 genuinely-important error-rules to warn. The corrected `buildA11yRules()` applies the recommended config properly: off-rules stay off, already-clean rules go to `error`, options preserved. The remaining backlog was then swept to zero in four batches, each rule promoted to `error` as its slice cleared. The approach was capability-first — where a keyboard or screen-reader user genuinely could not do something, it was fixed: a clickable `<div>` file-dropzone (whose file input was `display:none`, so keyboard users could not select a file at all) became a real `<button>`; an accordion header became a `role="button"` with `aria-expanded` and Enter/Space activation, its kebab menu moved to a sibling so an interactive control is not nested inside another; the mobile navigation menu gained Escape-to-close, where previously the only ways to dismiss it were a mouse backdrop click or selecting a nav item. Where the linter fired on genuinely non-interactive elements — `stopPropagation` event-containment wrappers, a click-to-focus wrapper around a keyboard-reachable editor — or on intentional focus-on-open (dialogs, popovers and inline forms the user just summoned, the ARIA Authoring Practices pattern for data-entry dialogs), each got a one-line justified disable. One real gap had no markup fix: author-uploaded lesson and resources videos render in native `<video>` with no captions, and there is no caption-upload flow to attach a real `<track>` to. Rather than fake it with an empty caption track — a false promise to assistive tech — that gap is acknowledged with justified disables and recorded as a Video captions feature backlog (optional `.vtt` upload, rendered as a `<track>` when present). Separately, a latent crash risk was hardened: undefended static-map lookups (`categoryConfig[dbKey]`) that throw when a database key drifts from a local config map were routed through a shared `getCategoryConfig()` helper with a neutral fallback; a project-wide `noUncheckedIndexedAccess` TypeScript flag was evaluated and declined, because of its 689 errors three-quarters sit in test files and one-off scripts and only ~142 in real app code, so the signal drowns in noise and it would block CI. And the learning celebration emails (certificate earned, course completed, welcome) moved from the old lime accent to the main app's brand green `#22A34B`, which needs a white logo for contrast, so those three regrouped to the dark-header, white-logo email family. |
