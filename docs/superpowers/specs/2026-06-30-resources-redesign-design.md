...
- **§4** The rail shows the **top two heading levels present** with scroll-spy: H2 + H3 on well-formed bodies, H1 + H2 for H1-sectioned Google Docs (the sanitiser does not demote levels), H2 + H4 when a document skips H3. A hardcoded H2/H3 filter was reviewed out (2026-07-02): it deleted the whole rail for those shapes. Deeper levels keep their anchors, they just aren't listed. No expand/collapse on the desktop rail, matching React.dev and MDN. (Below `lg` the rail becomes a collapsed disclosure, a separate reflow/zoom concern; see the §4 body.)
- **§4 reading measure (final 2026-07-02, settled through five live rounds: box → void → centred → spread → capped column).** The article content caps at **`max-w-[90ch]`** of the prose-sm font (~823px): text, grids and tables share one uniform right edge — no ragged "text stops midway", no 246px dead band beside a boxed TOC. The full-spread build ran first and the mandated prose check measured real policy text at 151ch/line (unreadable); a live three-way comparison (fill / prose-only cap / column cap) settled on the column cap because the prose-only cap re-opened the void on prose-only pages. The card's leftover width is a quiet gutter before the borderless rail — deliberate margin, not void. The `max-w-7xl` shell still caps the card (one-standard-width rule; capping the card itself was ruled out), and a 1366px Chromebook is unchanged since its column sits under the cap naturally. The `resource_grid` moves to `auto-fill` columns (3-up inside the measure; reflows without slivers at any width).
...
### Design
Replace the box with a rail: a left border track, level encoded by weight and size (H2 > H3), and one active marker. The active item carries a 3px teal left bar **and** teal bolder text; the "On this page" label is a bold, full-strength title (it's a different register from the nav links, so they don't compete). The rail shows the **top two heading levels present** (H2 + H3 on well-formed bodies; deeper levels still get anchor ids for deep links and search, they're just not listed). No expand/collapse: scroll-spy moves the marker, the list is stable.

Below 1024px the rail collapses to a real `<button aria-expanded>` "On this page" disclosure (the Shadcn Button primitive) between the title block and the content, instead of disappearing. The breakpoint split is pure CSS on always-rendered elements, so narrow viewports never flash the wrong control. Scroll-spy (`use-scroll-spy.ts`) and the glossary-filter heading-hiding (`visibleHeadings` in `native-article-view.tsx`) are preserved; `aria-current` stays on the active item, and the nav carries `aria-label="On this page"` so the landmark stays named.
...
**Ancestor fallback.** Feed the filtered rail list (`filterRailHeadings`) to `useScrollSpy`, not the full heading list: the hook's existing "last heading above the fold" gap-state branch then lands on the nearest listed ancestor when the reader is inside an unlisted deeper section. Ancestor fallback for free, no hook change. When the hook returns `""` (reader above the first heading) the rail marks nothing; a default-to-first marker was tried and cut in review because it asserts `aria-current` on a section the reader has not reached.

### Reading measure (added 2026-07-02)
Rebuilding the rail exposed the wide-screen whitespace problem (`memory/resources-toc-widescreen-whitespace.md`): the capped 720px body left a ~246px dead band beside the rail on a 1920px screen. Settled through five live rounds. The body column loses its own cap (`min-w-0 lg:flex-1`) and `ARTICLE_PROSE_CLASSES` carries the measure: `max-w-[720px]` → **`max-w-[90ch]`**, so text, grids and tables cap together on one uniform right edge, with the card's leftover reading as a deliberate gutter before the borderless rail. (A full uncapped spread was built first; the prose visual check measured 151ch/line on real policy text and a live three-way comparison chose the column cap, the prose-only alternative having re-opened the void on exactly the pages where reading matters.) The layout is a grid of three DOM-ordered children (`ARTICLE_LAYOUT_CLASSES`): header block, rail, content block. Below `lg` they stack, putting the disclosure between title and content; from `lg` the rail occupies the second column spanning both rows. The title/actions header spans its cell as card chrome; the breadcrumb sits above the whole layout at card level. Column-level siblings of the article (GlossaryFilter, empty states) carry the measure themselves. The `resource_grid` switches to `grid-cols-[repeat(auto-fill,minmax(min(210px,100%),1fr))]` so tile count follows the measure (3-up), never overflowing a column narrower than one tile. This resolves and retires the whitespace memo.

### Files
- `src/components/resources/article-outline.tsx` — rail, weight-by-level, teal marker (Option B), top-two-levels with scroll-spy, narrow-screen disclosure (Shadcn Button, CSS-only breakpoint).
...
- Component: one link per rail heading; active item has the marker and `aria-current`; nothing marked when scroll-spy is empty; named nav landmark; disclosure button wired with `aria-expanded`/`aria-controls` and collapsing on pick.
