...
**StatusBanner — investigated, NOT extracted.** The "Learning 4× duplicated status pills" didn't hold up. Enrolment-status pills (completed / in-progress / enrolled) live only in `course-card`. Due-date status renders in two places with different shapes — a compact `<Badge>` on `enrolled-course-card` vs a two-line alert block on the course detail page — so a shared component fits neither. The genuine repetition is the due-date *computation* (`daysUntilDue` + 7-day thresholds, ~12 lines × 2 sites); a `getDueStatus()` helper would DRY it but is too marginal to earn extraction, and the `<=14`-day list filters are a separate "upcoming" concept. `preview-mode-banner.tsx` is used in 3 places (not dead). No component extracted.
