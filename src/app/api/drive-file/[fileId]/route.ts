...
export async function GET(
  ...
  try {
    ...
    // PDFs, videos), attachment for everything else. Inline lets the
    // browser's native viewer render the file inside an iframe — used by
    // the news-feed document lightbox AND by the Resources article video
    // embed (iframe around /api/drive-file/{id} plays MP4s in Chrome's
    // native controls). Attachment triggers the download dialog.
