...
**Pass only the listed rail headings to `useScrollSpy`, and the ancestor marker comes for free (§4).** The rail lists the top two heading levels present (`filterRailHeadings`: H2 + H3 on well-formed bodies, H1 + H2 for H1-sectioned Google Docs, H2 + H4 when a document skips H3), and we feed that same filtered list to the hook. When the reader is deep inside an unlisted deeper section, no listed heading is in the band, so the gap-state fallback lands on the last listed heading above the fold, which is exactly the nearest listed ancestor. No level-to-parent mapping needed: filter before the hook, not after. When the hook returns `""` (reader above the first heading) the rail deliberately marks nothing; defaulting the marker to the first item was tried and cut because it asserts `aria-current` on a section the reader has not reached.

**The active reading-rail item uses a teal marker (3px teal left bar, teal text, bolder weight) on a borderless rail, not a `bg-accent` pill.** (§4, 2026-07-02, supersedes the earlier pill rule.) The rail has no border or box: a bold "On this page" label, a 2px track line down the list, the top two heading levels present (deeper levels keep their anchors but aren't listed), and scroll-spy moving a single teal marker. Why the reversal: the pill rule held while the TOC was a bordered panel, where brand colour would have competed with that second box. The rail is now borderless beside the measured content column, so the marker is the only chrome there and a teal bar reads as a clean position indicator rather than competition. Matches React.dev and MDN. See `src/components/resources/article-outline.tsx`.

**Below `lg` the rail is a collapsed "On this page" disclosure, not hidden (§4).** The old rail was `hidden lg:block`, so a user zoomed to 200% (WCAG reflow 1.4.10 narrows the effective viewport) lost the contents list entirely. The article layout is a grid of three DOM-ordered children (`ARTICLE_LAYOUT_CLASSES` in `article-constants.ts`): header block, ArticleOutline, content block. Below `lg` they stack, so the disclosure (a Shadcn `Button` ghost with `aria-expanded`/`aria-controls`) sits between the title and the content; from `lg` the nav places itself in the grid's second column spanning both rows as the sticky side rail. The breakpoint split is pure CSS (`lg:` variants on always-rendered elements), never a `matchMedia` state: a JS breakpoint duplicates Tailwind's `lg` value and, seeded desktop-first, paints the wrong control on narrow viewports before hydration catches up.

**Article content caps at the 90ch reading measure (§4).** `ARTICLE_PROSE_CLASSES` carries `max-w-[90ch]` (~829px at prose-sm): text, grids and tables cap together, one uniform right edge, and the card's leftover width is a deliberate gutter before the borderless rail. Don't "fix" the gutter by uncapping: the full spread was built and measured 151ch/line on real policy text (unreadable), and a prose-only cap (grids breaking out wide) re-opens a whitespace void on prose-only pages, exactly where reading matters most. The user chose the column cap after a live three-way comparison (2026-07-02). The layout cells themselves have no cap (the measure lives in the prose class), the title/actions header spans its cell as card chrome, and the breadcrumb sits above the whole layout at card level. Column-level siblings of the article (GlossaryFilter, empty states) carry `max-w-[90ch]` themselves so nothing runs past the measure's edge. The `resource_grid` uses `auto-fill` columns (`minmax(min(210px,100%),1fr)`), so it follows the measure (3-up), reflows without slivers, and never overflows a column narrower than one tile; never pin it to viewport breakpoints.
