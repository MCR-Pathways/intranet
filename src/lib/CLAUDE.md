...
**The header search bar centres over the feed by mirroring `<main>`, and shrinks to fit rather than hiding.** The header is full-width above the sidebar; the feed lives in `<main>` (offset `md:ml-64` / `md:ml-16`, centred in `max-w-7xl`). To line the bar up over the feed, its container reuses the same offset + `container` + 590px centre-column and reads the same `isCollapsed` (passed down from `AppLayout`); the logo and bell/avatar are `absolute` so they don't shove the centred bar. Its max-width is `min(590px, calc(100vw - 560px))`: the full 590px feed width whenever that fits (so it stays feed-aligned and full-size on Chromebooks and Retina Macs), narrowing only once the centred bar would reach the absolute clusters. The bar shows from `md`; below that the mobile layout takes over (a deferred responsive pass). The `560px` is the feed offset + corner reserve + gaps, plus a buffer because `100vw` counts the scrollbar (classic Windows scrollbars would otherwise shrink the gap to the bell). Resting fill is `bg-muted` (not `bg-card`) — a documented exception, since the bar sits on the white header where `bg-card` is invisible.
