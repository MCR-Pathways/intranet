/**
 * Certificate PDF generation using @react-pdf/renderer.
 *
 * Generates MCR Pathways branded certificates of completion.
 * Design: Coursera-inspired, left-aligned, typography-driven.
 * Server-only — uses Node.js APIs for PDF rendering.
 */

import React from "react";
import {
  Document,
  Page,
  Text,
  View,
  StyleSheet,
  Font,
  Svg,
  Path,
  ClipPath,
  Rect,
  Defs,
  G,
} from "@react-pdf/renderer";

// ─── Font Registration ──────────────────────────────────────────────────────

Font.register({
  family: "Playfair Display",
  src: "https://fonts.gstatic.com/s/playfairdisplay/v37/nuFvD-vYSZviVYUb_rj3ij__anPXJzDwcbmjWBN2PKd3vXDXbtM.ttf",
  fontWeight: 700,
});

// ─── Colours ────────────────────────────────────────────────────────────────

const NAVY = "#213350";
const GREY = "#6b7280";
const DARK_GREY = "#4b5563";
const LIGHT_GREY = "#9ca3af";
const BORDER_GREY = "#d1d5db";

// ─── Styles ─────────────────────────────────────────────────────────────────

const styles = StyleSheet.create({
  page: {
    flexDirection: "column",
    backgroundColor: "#FFFFFF",
    padding: 20,
    fontFamily: "Helvetica",
  },
  innerFrame: {
    flex: 1,
    border: `1.5pt solid ${BORDER_GREY}`,
    padding: 50,
    paddingTop: 40,
    paddingBottom: 30,
    flexDirection: "column",
  },
  logo: {
    marginBottom: 28,
  },
  certificateLabel: {
    fontSize: 11,
    color: GREY,
    letterSpacing: 3,
    textTransform: "uppercase",
    marginBottom: 20,
  },
  date: {
    fontSize: 12,
    color: GREY,
    fontStyle: "italic",
    marginBottom: 20,
  },
  recipientName: {
    fontSize: 32,
    fontFamily: "Playfair Display",
    fontWeight: 700,
    color: NAVY,
    marginBottom: 16,
  },
  completionText: {
    fontSize: 12,
    color: DARK_GREY,
    marginBottom: 12,
  },
  courseTitle: {
    fontSize: 18,
    fontWeight: "bold",
    color: NAVY,
    marginBottom: 0,
  },
  spacer: {
    flex: 1,
  },
  certNumber: {
    fontSize: 10,
    color: LIGHT_GREY,
    textAlign: "right",
  },
});

// ─── MCR Logo (inline SVG paths) ───────────────────────────────────────────

const MCR_LOGO_PATHS = [
  "M79.6282 0.898205H85.944V20.5549H79.8517V11.4571L75.8088 20.5549H73.2824L69.2954 11.5409V20.5549H63.2031V0.898205H69.6327L74.6294 12.6068L79.6262 0.898205H79.6282Z",
  "M87.5156 10.7265C87.5156 5.13772 91.9237 0.477039 97.7626 0.477039C105.062 0.477039 107.167 6.17764 107.477 8.08582L101.245 8.70259C100.907 7.80438 99.9536 6.09181 97.7646 6.09181C95.5755 6.09181 93.7496 7.91616 93.7496 10.7245C93.7496 13.5329 95.5735 15.3573 97.7646 15.3573C99.9556 15.3573 100.882 13.7285 101.245 12.7465L107.477 13.3932C107.223 14.994 105.062 20.974 97.7626 20.974C91.9237 20.974 87.5156 16.3134 87.5156 10.7245V10.7265Z",
  "M120.138 20.5549L116.741 14.6587H115.478V20.5549H109.246V0.898205H118.651C123.199 0.898205 126.146 3.84631 126.146 7.77844C126.146 10.1657 124.911 12.1018 122.75 13.3094L126.988 20.5549H120.138ZM115.197 5.81238V9.88423H117.639C118.93 9.88423 119.772 9.04192 119.772 7.83433C119.772 6.62675 118.93 5.81238 117.639 5.81238H115.197Z",
  "M69.5132 25.1497C73.0513 25.1497 75.829 27.8463 75.829 31.2156C75.829 34.5848 73.0493 37.3094 69.5132 37.3094H65.2468V44.8064H63.2812V25.1497H69.5132ZM69.5132 35.483C72.0116 35.483 73.8654 33.6866 73.8654 31.2156C73.8654 28.7445 72.0136 26.976 69.5132 26.976H65.2468V35.485H69.5132V35.483Z",
  "M87.7598 35.6806V44.8064H86.4129L85.9359 42.8962H85.7962C85.2355 43.7665 83.8307 45.0858 81.474 45.0858C78.5825 45.0858 76.8145 43.1477 76.8145 40.8743C76.8145 38.2914 79.3128 36.493 82.9347 36.493H85.9379V35.6786C85.9379 33.5729 84.563 32.1976 82.5695 32.1976C80.576 32.1976 79.5383 33.3493 79.257 34.8363H77.4331C77.7144 32.505 79.7359 30.483 82.5695 30.483C85.5727 30.483 87.7638 32.6168 87.7638 35.6786L87.7598 35.6806ZM85.9359 39.3593V38.1517H83.2121C80.3485 38.1517 78.6364 39.1337 78.6364 40.8763C78.6364 42.3373 79.676 43.3753 81.6675 43.3753C84.3056 43.3753 85.9339 41.6347 85.9339 39.3593H85.9359Z",
  "M91.9984 40.5948V32.4791H89.5V30.7665H90.5955C91.719 30.7665 92.3077 30.2056 92.3077 29.0819V27.032H93.8243V30.7665H97.1089V32.4791H93.8243V40.6228C93.8243 42.533 94.7502 43.3753 96.155 43.3753C96.5202 43.3753 96.8016 43.3194 97.1089 43.2076V44.8643C96.7996 44.9761 96.4623 45.0898 95.8737 45.0898C93.6846 45.0898 92.0004 43.4611 92.0004 40.5968L91.9984 40.5948Z",
  "M99.9453 44.8064V25.1497H101.769V32.6747H101.909C102.528 31.7206 103.958 30.4851 106.036 30.4851C109.208 30.4851 111.623 32.9002 111.623 36.6627V44.8064H109.799V36.6627C109.799 33.8543 108.143 32.1976 105.868 32.1976C103.453 32.1976 101.769 33.8543 101.769 36.6627V44.8064H99.9453Z",
  "M129.618 30.7665H131.499L126.812 44.8064H125.493L122.152 34.1078L118.868 44.8064H117.549L112.861 30.7665H114.827L118.223 41.521L121.564 30.7665H122.827L126.167 41.521L129.62 30.7665H129.618Z",
  "M142.924 35.6806V44.8064H141.577L141.1 42.8962H140.96C140.4 43.7665 138.995 45.0858 136.636 45.0858C133.745 45.0858 131.977 43.1477 131.977 40.8743C131.977 38.2914 134.475 36.493 138.097 36.493H141.1V35.6786C141.1 33.5729 139.725 32.1976 137.732 32.1976C135.738 32.1976 134.701 33.3493 134.419 34.8363H132.595C132.877 32.505 134.898 30.483 137.732 30.483C140.735 30.483 142.926 32.6168 142.926 35.6786L142.924 35.6806ZM141.1 39.3593V38.1517H138.376C135.513 38.1517 133.801 39.1337 133.801 40.8763C133.801 42.3373 134.84 43.3753 136.832 43.3753C139.47 43.3753 141.098 41.6347 141.098 39.3593H141.1Z",
  "M155.165 30.7665H157.075L150.871 46.2096C149.608 49.3274 148.456 50 147.082 50C146.633 50 146.295 49.8882 145.986 49.7765V48.1198C146.295 48.2315 146.575 48.2874 146.856 48.2874C147.698 48.2874 148.401 47.8383 148.989 46.4052L149.748 44.495L144.162 30.7645H146.128L150.703 42.1657L155.167 30.7645L155.165 30.7665Z",
  "M157.805 40.7345H159.628C159.796 42.2794 160.975 43.3733 162.969 43.3733C164.962 43.3733 165.944 42.3633 165.944 41.0978C165.944 37.6727 158.168 39.2735 158.168 34.4152C158.168 32.2535 159.964 30.483 162.771 30.483C165.579 30.483 167.431 32.3932 167.573 34.6946H165.749C165.581 33.1218 164.541 32.1956 162.773 32.1956C161.005 32.1956 159.994 33.1497 159.994 34.3293C159.994 37.6427 167.77 36.1836 167.77 41.0958C167.77 43.3134 166.002 45.0838 162.969 45.0838C159.936 45.0838 157.972 42.978 157.805 40.7305V40.7345Z",
  "M37.6473 0C24.2175 0 23.4552 0.762475 23.4552 14.1956V19.5549C23.4552 32.988 24.2175 33.7505 37.6473 33.7505C38.216 33.7505 38.7608 33.7505 39.2856 33.7445L31.4991 44.7425C48.1995 44.7425 51.8374 41.1038 51.8374 24.3992V14.1956C51.8394 0.762475 51.0771 0 37.6473 0Z",
  "M9.81791 2.79641C0.526815 2.79641 0 3.32335 0 12.6168V19.6767C0 31.2335 2.51833 33.7505 14.0703 33.7505L8.68446 26.1417C9.04764 26.1437 9.42479 26.1457 9.81791 26.1457C19.109 26.1457 19.6358 25.6188 19.6358 16.3254V12.6168C19.6358 3.32335 19.109 2.79641 9.81791 2.79641Z",
];

function MCRLogo() {
  return React.createElement(
    Svg,
    { width: 140, height: 42, viewBox: "0 0 168 50" },
    React.createElement(
      Defs,
      null,
      React.createElement(
        ClipPath,
        { id: "clip0" },
        React.createElement(Rect, { width: "168", height: "50", fill: "white" })
      )
    ),
    React.createElement(
      G,
      { clipPath: "url(#clip0)" },
      ...MCR_LOGO_PATHS.map((d, i) =>
        React.createElement(Path, { key: String(i), d, fill: NAVY })
      )
    )
  );
}

// ─── Certificate Document ───────────────────────────────────────────────────

export interface CertificateData {
  recipientName: string;
  courseTitle: string;
  completionDate: string;
  certificateNumber: string;
}

/**
 * React PDF Document component for certificate generation.
 * Design: Coursera-inspired, left-aligned, Playfair Display serif for name.
 */
export function CertificateDocument({ data }: { data: CertificateData }) {
  return React.createElement(
    Document,
    null,
    React.createElement(
      Page,
      { size: "A4", orientation: "landscape", style: styles.page },
      React.createElement(
        View,
        { style: styles.innerFrame },
        // MCR logo
        React.createElement(View, { style: styles.logo }, React.createElement(MCRLogo)),
        // Certificate label
        React.createElement(
          Text,
          { style: styles.certificateLabel },
          "CERTIFICATE OF COMPLETION"
        ),
        // Completion date
        React.createElement(
          Text,
          { style: styles.date },
          data.completionDate
        ),
        // Recipient name (serif, large)
        React.createElement(
          Text,
          { style: styles.recipientName },
          data.recipientName
        ),
        // Body text
        React.createElement(
          Text,
          { style: styles.completionText },
          "has successfully completed"
        ),
        // Course title
        React.createElement(
          Text,
          { style: styles.courseTitle },
          data.courseTitle
        ),
        // Spacer pushes certificate number to bottom
        React.createElement(View, { style: styles.spacer }),
        // Certificate number (bottom-right)
        React.createElement(
          Text,
          { style: styles.certNumber },
          `Certificate ${data.certificateNumber}`
        )
      )
    )
  );
}

/**
 * Generate a certificate PDF as a Buffer.
 * Server-only — calls @react-pdf/renderer.
 */
export async function generateCertificatePdf(
  data: CertificateData
): Promise<Buffer> {
  const { renderToBuffer } = await import("@react-pdf/renderer");
  const element = React.createElement(CertificateDocument, { data });
  // @react-pdf/renderer typing expects ReactElement<DocumentProps> but
  // createElement returns ReactElement<unknown>. The element IS a Document.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const buffer = await renderToBuffer(element as any);
  return Buffer.from(buffer);
}
