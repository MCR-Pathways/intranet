...
**An outer try/catch can't see through inner catches that swallow errors.** A common pattern wraps each part of a multi-step job in its own try/catch so that one block failing doesn't abort the others ("do as much as you can"). If you then wrap everything in an outer try to write an audit row, the outer catch is dead code for any failure the inner catches already handled, and the success path runs as if nothing went wrong. The audit row lies. Track a per-block failure flag inside each inner catch and use it when finalising the audit row. PR #275 hit this with `daily-reminders`: both the L&D and HR blocks had inner try/catch, and the outer path wrote `cron_runs.status = 'success'` whether or not anything ran successfully. Now a `blockErrors` array flips the status to `'failed'` when any block crashes, with the error text captured in `cron_runs.result.blockErrors` and `cron_runs.error`.
...
**Capability flags and data-access roles are different policy decisions — keep them decoupled.** Don't gate "can do action X" on a permission flag that was set up for "can see data Y". W4b would have used `is_hr_admin` to gate posting announcements; Colin pushed back because HR-data permissions and broadcast-comms authority are independent concerns (changing one shouldn't change the other, and giving HR admins broadcast power risks announcements containing PII because the same person has access to both). The pattern: a new boolean column on `profiles` (e.g. `can_post_announcements`), default false, granted by a strictly-superior role (e.g. `is_systems_admin`). Defence-in-depth: both the UI gate AND the server action's whitelist must strip the field if the caller lacks the granting role. Captured here even though Announcements (`memory/announcement-deferred.md`) didn't ship — the principle survives the scratch.

**Sequential, not parallel, when one server action reads what another writes.** With `Promise.all([writeA, readB])` where readB's side effects depend on writeA's commit (e.g. a downstream notification reading the post's content), parallel ordering races the SELECT against the UPDATE. If the SELECT lands first, the notification carries the OLD content while the post itself eventually shows the NEW one — permanently wrong notification, no error. PR #299's edit-mode handleSubmit hit this: `editPost` + `addKudosRecipients` ran via `Promise.all`, but `addKudosRecipients` reads `posts.content` server-side to populate the new recipient's bell row. Switch to sequential `await writeA; await readB` whenever the second action has any read of state the first action just changed. The cost (one extra round-trip) is cheap; the alternative (permanently wrong downstream state, no error surfaced) is uncatchable in QA.

**Compute-on-read for time-bound visual state, not cron.** When chrome should drop at a deadline (e.g. an active announcement → regular post visually at `expires_at`), encode the "is this active?" check in the renderer rather than scheduling a cron to flip a flag. Pattern: `is_pinned AND (post_type != 'announcement' OR announcement_expires_at > now())`. Expiry is a function of current time on every render — no drift possible, no cron job to maintain, no race between the cron firing and the user opening the page. Generalises to any "this state holds until X" visual concern. Source-of-truth columns stay simple (boolean + timestamp); the temporal logic lives at the read site. Originally W4b's expiry handling; preserved here regardless because the pattern is broadly useful.

**`.trim() ||` over `??` for env-var fallbacks where empty/whitespace is invalid.** `process.env.NEXT_PUBLIC_APP_URL ?? "https://intranet.mcrpathways.org"` doesn't fall back when the env var is set to an empty string — `??` only triggers on `null`/`undefined`. For URL hosts, empty-string is exactly as broken as undefined. `||` alone catches the empty-string case but a whitespace-only string `" "` is truthy and would still slip through. The combined pattern that handles all three failure modes (undefined / empty / whitespace-only) is `process.env.X?.trim() || "fallback"` — `.trim()` collapses whitespace-only to empty-string, `||` then falls back. Memoised via Gemini review on the scratched W4b PR; same fix shape applies anywhere an env var is consumed with a sensible default.

**Memoise inline-literal props passed to memoised children.** Passing `editTarget={{ postId: ..., message: ..., existingRecipients: [...] }}` directly in JSX recreates the object on every parent render. If the child has `useMemo` deps or `useEffect` deps that include the prop, those re-run for no reason — the child sees a "changed" object identity even when nothing actually changed. Wrap parent-side: `const editTarget = useMemo(() => ({ ... }), [post.id, post.content, ...])`. Knock-on: any array literal handed in via `?? []` also needs its own `useMemo` so the editTarget memo's deps stay stable. Caught by Gemini on PR #299; the W4b PR repeated the pattern before being scratched.
