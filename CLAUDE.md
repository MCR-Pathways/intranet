...
**Global search uses Cmd+K overlay, not per-module search bars.** A search icon in the header opens a centered command palette overlay. Uses Algolia multi-index query across `resources_articles` + `learning_courses`. Results grouped by type. See `global-search.tsx` and `src/lib/algolia.ts` for index constants. The Resources landing page has a search prompt that programmatically opens the global overlay via `KeyboardEvent` dispatch — no duplicate Algolia widget.

**Don't duplicate navigation information between sidebar and page body.** When a sidebar tree shows the same categories that appear as cards on the page, users see the same information twice. Choose one: either the sidebar provides navigation (Notion/Confluence pattern) or the page body provides navigation (Zendesk/Document360 pattern). For lookup-oriented knowledge bases (~80 staff, mostly policies), the page-body pattern with breadcrumbs + search is sufficient.

**Use grouped index sections on category pages, not subcategory cards + flat article list.** Showing subcategory cards at the top AND a flat article list below creates two problems: (1) the cards duplicate what's already navigable via breadcrumbs, (2) the flat list doesn't show which articles belong to which subcategory. Instead, show subcategories as expandable sections with their articles inline (GitBook/Document360 pattern). Hide empty subcategories.

**Hide empty categories from end users.** Categories with 0 published articles should not appear in the category grid or grouped index. Nearly every knowledge base platform (Zendesk, Freshdesk, HubSpot, Document360, BoldDesk) hides empty containers from readers. Show them only to editors.

**Use "More in [folder]" footer on article pages instead of a sidebar tree.** When removing the sidebar tree, sibling article navigation is lost. A "More in [category]" section at the bottom of article pages lists other articles in the same folder, with the current article highlighted. This solves the discovery problem without a persistent sidebar. Zendesk and Help Scout use this pattern.

**Use custom DOM events, not synthetic KeyboardEvents, for cross-component communication.** Synthetic `KeyboardEvent` dispatch is fragile — browsers may not propagate them identically to real events. Use `new CustomEvent("open-global-search")` and listen with `document.addEventListener`. The global search component listens for both the custom event AND real Cmd+K/Ctrl+K keydown.

**Use `suppressHydrationWarning` for platform-specific rendering.** Values that depend on `navigator.platform` (e.g. showing ⌘K vs Ctrl+K) differ between server and client. Instead of `useState` + `useEffect` (which the React Compiler flags), compute inline with `typeof window !== "undefined"` and add `suppressHydrationWarning` to the element.

**Don't nest interactive elements — use sibling layout instead.** A `<Link>` inside a `<button>` is invalid HTML and confuses screen readers. Split into sibling elements: `<Link>` for navigation + `<button>` for toggle, separated by `border-l`. Add `aria-label` to the button for screen reader context.

**Remove monolithic card wrappers — let each page manage its own surfaces.** A single `bg-card` shell wrapping all page content (landing, category, article) creates a monolithic white block. Other modules (Learning) use the grey `bg-background` to visually separate sections. Remove the shell wrapper and let each page decide its own card surfaces.

**Add `shadow-sm` to cards on grey backgrounds for accessibility.** The default `border-border` (#e5e7eb) on `bg-background` (#F2F4F7) is near-invisible — cards blend into the page. Adding `shadow-sm` (matching the Shadcn Card component default) lifts cards off the background. Use `hover:shadow-md` for interactive uplift.

**Use IntersectionObserver for TOC scroll-spy.** When a document has heading IDs, add an IntersectionObserver with `rootMargin: "-80px 0px -80% 0px"` to detect the topmost visible heading. Highlight it in the TOC with `text-primary font-medium`. Smooth-scroll on TOC click. See `article-outline.tsx` for the reusable Tiptap version and `google-doc-article-view.tsx` for the HTML version.

**Research platforms before redesigning, then cherry-pick the best of each.** The Resources redesign researched 5 platforms (Outline, Slab, Mintlify, Slite, Notion), verified claims against actual products, identified what to take and what NOT to take, then combined the best patterns. Unverified claims (e.g. "Mintlify uses Algolia" — false, they moved to their own search) would have led to wrong decisions. Always verify before acting on inspiration.

**Evaluate cherry-picked features for overkill before implementing.** Personal bookmarks, category maintainers, and Cmd+K quick actions were all cherry-picked from platforms but rejected as overkill for ~80 staff. Always ask: does the scale justify the complexity? If not, note it for later.
