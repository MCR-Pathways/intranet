...
2. Build, run the `/code-review` skill on the diff, address what it finds, then commit. This is hard-enforced: a `PreToolUse(Bash)` hook (`.claude/hooks/require-code-review.mjs`) blocks `git commit` (docs as well as code) unless `/code-review` has run since the last commit. A commit message containing `[skip-review]` bypasses the gate for trivial or emergency commits.
