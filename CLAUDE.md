...
**Sweep for same-pattern issues when accepting a Gemini review fix.** Reviewers typically flag a pattern in one file and don't catch identical uses elsewhere. Before pushing the fix, grep for the same pattern across the codebase. Missing the sibling occurrence costs another review round and another PR cycle. Applied this after PR #260: Gemini flagged a null expiration fallback in the cron handler; the same pattern existed unflagged in `linkGoogleDoc`.
