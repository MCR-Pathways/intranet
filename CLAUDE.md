...
**`||` over `??` for env-var fallbacks where empty string is invalid.** `process.env.NEXT_PUBLIC_APP_URL ?? "https://intranet.mcrpathways.org"` doesn't fall back when the env var is set to an empty string — `??` only triggers on `null`/`undefined`. For URL hosts, empty-string is exactly as broken as undefined. Use `||` (truthy check) so empty/whitespace-only values also route to the fallback. Memoised via Gemini review on the scratched W4b PR; same fix shape applies anywhere an env var is consumed with a sensible default.
